...
These configurations allow you to control request limits per consumer and restrict access by IP ranges — something essential for internal or sensitive APIs.

## 🎛️ Creating plugins as separate objects
...
## 🔗  Attaching plugins via annotation in Ingress
...
## 📍 Using exact paths in Ingress

I configured Ingress with an exact path—which prevents unexpected coincidences in routes:
...
## 🚫 Understanding `strip-path: “false”`
...
In my case, the backend needed the full path (to restrict to a specific endpoint) — so:

```yaml
konghq.com/strip-path: "false"
```

## 🧠 What really made everything “click”
